...
type TabType = 'warehouses' | 'packages';
...
export const Settings = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      // Fetch warehouses from root warehouses collection
      ...
      // Fetch packages from root templates collection
  ...
  const handleWarehouseSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // SIMPLE: Direct path - warehouses/{docId}
  ...
  const deleteWarehouse = async (id: string) => {
    ...
    try {
      // SIMPLE: Direct path - warehouses/{id}
  ...
  const handlePackageSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // SIMPLE: Direct path - templates/{docId}
  ...
  const deletePackage = async (id: string) => {
    ...
    try {
      // SIMPLE: Direct path - templates/{id}
  ...
  return (
    <div className="flex-1 bg-slate-50 p-8 min-h-screen font-sans text-slate-900 overflow-y-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-2">Manage your warehouses and package templates.</p>
      </div>
