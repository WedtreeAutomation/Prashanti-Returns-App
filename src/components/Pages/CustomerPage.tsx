...
import { fetchOrderByName, fetchOrdersByCustomer, fetchProductDetails, fetchShopifyOrder, notifyReturnSubmission } from '../../Interfaces/api';
...
const RETURN_DAYS_LIMIT = 14; // 14 days return window
...
type Step = 'LOGIN' | 'ORDER_SELECTION' | 'SELECT' | 'METHOD' | 'REVIEW' | 'CONFIRMATION';
...
/**
 * Check if order is within 14 days from creation date
 */
const isWithinReturnPeriod = (created_at: string): boolean => {
  ...
  // Allow returns within 14 days (inclusive)
 ...
 */
const getDaysSinceOrder = (created_at: string): number => {
...
const ImageCarousel = ({ images }: { images: string[] }) => {
  ...
  return (
    <div className="relative aspect-square rounded-lg border border-slate-200 overflow-hidden bg-slate-50 group">
      <img 
...
const MobileHeader = ({ 
  ...
  const getStepTitle = () => {
    switch(step) {
      ...
      case 'ORDER_SELECTION': return 'Select Order';
    ...
    <div className="lg:hidden bg-white border-b border-slate-200 sticky top-0 z-50">
      <div className="px-4 py-3 flex items-center justify-between">
        <Link to="/CustomerPage" className="flex items-center gap-2">
          ...
          <span className="font-bold text-sm">
            Prashanti<span className="text-pink-600">Returns</span>
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/ReturnStatusPage"
            className="text-pink-600 p-2 hover:bg-pink-50 rounded-lg"
          >
      ...
      <div className="px-4 pb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-pink-600">{getStepTitle()}</span>
        </div>
      </div>
    </div>
  );
...
export const ReturnPortalSplit = () => {
  ...
  // Global order check - blocks Final Sale items AND orders older than 14 days
  const isOrderEligibleForReturn = useCallback((order: Order): { eligible: boolean; reason?: 'final_sale' | 'time_window' | 'active_return' | 'international' } => {
    // Check 1: Time window (14 days)
    if (!isWithinReturnPeriod(order.created_at)) {
      return { eligible: false, reason: 'time_window' };
    }
    
    // Check 2: Final sale tags
  ...
  }, [customerOrders, checkOrderReturnStatus]);
  ...
  const handleHistorySearch = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      const orders = await fetchOrdersByCustomer(historyEmail) || [];
      if (orders?.length) {
        setCustomerOrders(orders);
        setStep('ORDER_SELECTION');
      } else {
        setError("No fulfilled orders found for this contact info.");
      }
    } catch (err) {
  ...
  const selectOrderFromHistory = useCallback(async (selectedOrder: Order) => {
    const eligibility = isOrderEligibleForReturn(selectedOrder);
    
    if (!eligibility.eligible) {
      if (eligibility.reason === 'time_window') {
        showIneligibleModal(selectedOrder.name, 'time_window');
      } else if (eligibility.reason === 'final_sale') {
        showIneligibleModal(selectedOrder.name, 'custom');
      }
      return;
    }

    const isInIndia = await isOrderInIndia(selectedOrder);
    if (!isInIndia) {
      showIneligibleModal(selectedOrder.name, 'international');
      return;
    }

    setHistoryLoading(true);
    await loadOrderProductDetails(selectedOrder);
    setOrder(selectedOrder);
    setStep('SELECT');
    setHistoryLoading(false);
  }, [isOrderEligibleForReturn, isOrderInIndia, loadOrderProductDetails, showIneligibleModal]);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        <div className="relative bg-white rounded-2xl max-w-md w-full p-6 sm:p-8 shadow-2xl animate-in zoom-in-95 mx-4">
          ...
          <div className={`w-16 h-16 sm:w-20 sm:h-20 ${
            ...
            isTimeWindow ? 'bg-orange-100' : 'bg-red-100'
            ...
            {isInternational ? (
              ...
              <AlertCircle className={`w-8 h-8 sm:w-10 sm:h-10 ${isActiveReturn ? 'text-amber-600' : 'text-red-600'}`} />
            )}
          </div>
          ...
          <div className="text-center mb-4 sm:mb-6">
            <p className="text-sm sm:text-base text-slate-700 mb-2">
              Order <span className="font-bold text-pink-600">{ineligibleModal.orderName}</span>
            </p>
            <div className={`${
              isInternational ? 'bg-blue-50 border-blue-100' :
              isActiveReturn ? 'bg-amber-50 border-amber-100' :
              isTimeWindow ? 'bg-orange-50 border-orange-100' : 'bg-red-50 border-red-100'
            } border rounded-xl p-3 sm:p-4`}>
              <p className={`${
                isInternational ? 'text-blue-700' :
                isActiveReturn ? 'text-amber-700' :
                isTimeWindow ? 'text-orange-700' : 'text-red-700'
                ...
                {isInternational
                  ...
                  : isActiveReturn
                    ...
                    : isTimeWindow
                      ? 'Returns are only accepted within 14 days of order delivery'
                      : 'This order is marked as Final Sale'
                }
              </p>
              {isTimeWindow && (
                <p className="text-xs sm:text-sm text-orange-600 mt-2">
                  Please contact customer support for assistance with this order.
                </p>
              )}
            </div>
            <p className="text-xs sm:text-sm text-slate-600 mt-3 sm:mt-4">
              {isInternational
                ...
                : isActiveReturn
                  ...
                  : isTimeWindow
                    ? `Our return policy allows returns within ${RETURN_DAYS_LIMIT} days from the date of delivery.`
          ...
          <button
            onClick={() => setIneligibleModal({ show: false, orderName: '', type: 'custom' })}
            className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 px-4 rounded-xl transition-all text-sm sm:text-base"
          >
            Got it
          </button>
        </div>
  ...
  const renderItemCard = useCallback((item: any) => {
    ...
    const daysSinceOrder = order ? getDaysSinceOrder(order.created_at) : 0;
    const isNearExpiry = daysSinceOrder >= 12 && daysSinceOrder <= 14;
    ...
    return (
      ...
      >
            ...
            <div className={`w-5 h-5 sm:w-6 sm:h-6 rounded-lg border-2 flex items-center justify-center transition-colors ${
              isDisabled
                ...
                : isSelected
                  ...
                  : 'bg-white border-slate-300 hover:border-pink-300 cursor-pointer'
            }`}>
              ...
              <div className="mt-2 sm:mt-3 bg-orange-50 border border-orange-200 rounded-lg p-2 sm:p-3">
                <div className="flex items-start gap-1.5 sm:gap-2">
                  ...
                  <p className="text-[10px] sm:text-xs text-orange-700">
                    Return window closes in {RETURN_DAYS_LIMIT - daysSinceOrder} day(s). Please submit soon.
                  </p>
                </div>
              </div>
            )}
        ...
        {isSelected && !isDisabled && (
          <div className="px-3 sm:px-5 pb-4 sm:pb-6 pt-2 bg-slate-50/50 border-t border-pink-100">
            <div className="grid grid-cols-1 gap-3 sm:gap-4 mb-4 sm:mb-6">
              <div>
                ...
                <select
                  ...
                  className="w-full p-2 sm:p-2.5 text-sm bg-white border border-slate-200 rounded-xl outline-none focus:border-pink-500 focus:ring-2 focus:ring-pink-100"
                >
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-slate-500 mb-2 sm:mb-3">
                <span className="text-pink-600 font-bold">{MANDATORY_IMAGES} required</span> • Max {MAX_IMAGES} • 5MB each
              </p>
                      ...
                      {hasFile ? (
                        ...
                        <label
                          ...
                          className={`
                            ...
                            ${isRequired
                              ? 'border-pink-200 bg-pink-50/50 hover:bg-pink-100'
                        ...
                        >
                          <Camera className={`w-3 h-3 sm:w-5 sm:h-5 mb-0.5 sm:mb-1 ${
                            isRequired ? 'text-pink-400' : 'text-slate-400'
                          }`} />
                          <span className={`text-[6px] sm:text-[10px] font-bold ${
                            isRequired ? 'text-pink-600' : 'text-slate-400'
                          }`}>
  ...
  }, [selectedItems, returnDetails, uploadFiles, previews, productDetails, order, toggleItem, updateDetail, handleFileChange, removeImage, canItemBeReturned, isItemCustomized]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 relative">
      ...
      {/* Desktop Header */}
      <div className="hidden lg:block bg-white border-b border-slate-200 shadow-sm sticky top-0 z-50">
        <div className="w-full px-16 py-4">
          <div className="flex items-center justify-between w-full">
            <Link to="/CustomerPage" className="flex items-center gap-3 group">
              <div className="w-10 h-10 bg-gradient-to-br from-pink-500 to-purple-600 rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all">
                <PackageOpen className="w-5 h-5 text-white" />
              </div>
              <div>
                <span className="font-bold text-lg text-slate-900">Prashanti</span>
                <span className="text-pink-600 font-bold text-lg ml-1">Returns</span>
              </div>
            </Link>
            <div className="flex items-center gap-4">
              <Link
                to="/ReturnStatusPage"
                className="flex items-center gap-2 text-pink-600 hover:text-pink-700 transition-colors px-4 py-2 rounded-lg hover:bg-pink-50 border border-pink-200"
              >
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-x-0 top-[73px] bg-white border-b border-slate-200 shadow-lg z-40 animate-in slide-in-from-top">
          <div className="p-4 space-y-2">
            <Link
              to="/ReturnStatusPage"
              className="flex items-center gap-3 p-3 text-pink-600 hover:bg-pink-50 rounded-xl transition-colors"
      ...
      <div className="flex flex-col lg:flex-row flex-1">
        {/* LEFT PANEL */}
        <div className={`
          ${step === 'LOGIN' || step === 'ORDER_SELECTION' 
            ? 'block w-full lg:w-[450px]' 
            : 'hidden lg:block lg:w-[450px]'
          } 
          bg-white border-r border-slate-200 flex-col h-auto lg:min-h-[calc(100vh-73px)] shadow-2xl z-20
        `}>
          ...
          <div className="flex-1 px-4 sm:px-6 lg:px-8 py-2 sm:py-4 flex flex-col gap-4 sm:gap-6 overflow-y-auto">
            {step === 'LOGIN' ? (
              <div className="space-y-4 sm:space-y-6">
                <div className="bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm">
                  <div className="flex items-center gap-2 mb-3 sm:mb-4 text-slate-800">
                    ...
                    <h3 className="font-bold text-xs sm:text-sm uppercase tracking-wide">Find Specific Order</h3>
                  </div>
                  <form onSubmit={handleSpecificSearch} className="space-y-2 sm:space-y-3">
                    <input
                      ...
                      className="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm bg-white border border-slate-200 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-pink-100 focus:border-pink-500 outline-none transition-all"
                      disabled={isSpecificLoading || isHistoryLoading}
                    />
                    <input
                      type="text"
                      value={specificEmail}
                    ...
                    <button
                      type="submit"
                      disabled={isSpecificLoading || isHistoryLoading}
                      className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-2 sm:py-3 rounded-lg sm:rounded-xl transition-all flex items-center justify-center gap-2 mt-1 sm:mt-2 text-xs sm:text-sm disabled:opacity-70 disabled:cursor-not-allowed"
                    >
                      {isSpecificLoading ? (
                        <>
                ...
                <div className="flex items-center gap-3 sm:gap-4">
                  ...
                  <span className="text-slate-400 text-[10px] sm:text-xs font-bold uppercase">OR</span>
                  <div className="h-px bg-slate-200 flex-1"></div>
                </div>

                <div className="bg-white border-2 border-pink-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm hover:border-pink-100 transition-colors">
                  <div className="flex items-center gap-2 mb-3 sm:mb-4 text-slate-800">
                    <History className="w-4 h-4 sm:w-5 sm:h-5 text-pink-600" />
                    <h3 className="font-bold text-xs sm:text-sm uppercase tracking-wide">View Past Orders</h3>
                  </div>
                  <form onSubmit={handleHistorySearch} className="space-y-2 sm:space-y-3">
                    <input
                      type="text"
                      value={historyEmail}
                      onChange={e => setHistoryEmail(e.target.value)}
                      placeholder="Email or Phone Number"
                      className="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-pink-100 focus:border-pink-500 outline-none transition-all"
                      disabled={isSpecificLoading || isHistoryLoading}
                    />
                    <button
                      type="submit"
                      disabled={isSpecificLoading || isHistoryLoading}
                      className="w-full bg-white border-2 border-pink-100 hover:border-pink-200 text-pink-600 font-bold py-2 sm:py-3 rounded-lg sm:rounded-xl transition-all flex items-center justify-center gap-2 mt-1 sm:mt-2 text-xs sm:text-sm disabled:opacity-70 disabled:cursor-not-allowed"
                    >
                      {isHistoryLoading ? (
                        <>
                          <Loader2 className="w-3 h-3 sm:w-4 sm:h-4 animate-spin" />
              ...
              <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-left-4">
                <div className="bg-slate-50 p-3 sm:p-5 rounded-xl sm:rounded-2xl border border-slate-200">
                  ...
                  <p className="text-xs sm:text-sm font-medium text-slate-800 break-all">{historyEmail}</p>
                  <button
                    onClick={resetForm}
                    className="mt-2 sm:mt-3 text-[10px] sm:text-xs font-bold text-pink-600 hover:underline flex items-center gap-1"
                  >
                    <ArrowLeft className="w-3 h-3" /> Sign Out
                  </button>
                </div>
              </div>
            ) : step === 'SELECT' && order ? (
              <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-left-4">
                ...
                <button
                  onClick={resetForm}
                  className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm font-semibold text-slate-400 hover:text-pink-600 transition-colors px-2"
                >
        ...
        <div className={`
          ...
          ${(step !== 'LOGIN' && step !== 'ORDER_SELECTION') ? 'block' : 'lg:block'}
        `}>
          ...
          {step === 'LOGIN' && (
            <div className="h-full flex flex-col items-center justify-center text-center opacity-40 select-none py-8 sm:py-12">
              ...
              <p className="max-w-[250px] sm:max-w-xs mt-1 sm:mt-2 text-xs sm:text-sm text-slate-400 px-4">
                Enter your details on the left to find your eligible items.
              </p>
            </div>
          )}
          ...
          {step === 'ORDER_SELECTION' && (
            <div className="max-w-3xl mx-auto space-y-4 sm:space-y-6">
              <h2 className="text-lg sm:text-xl font-bold text-slate-900 px-2 sm:px-0">Select an order to return</h2>
              <div className="grid gap-3 sm:gap-4">
                {customerOrders.map((o) => {
                  const eligibility = isOrderEligibleForReturn(o);
                  const orderStatus = orderReturnStatus[o.name];
                  const daysSinceOrder = getDaysSinceOrder(o.created_at);
                  const isExpired = daysSinceOrder > RETURN_DAYS_LIMIT;
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        if (eligibility.eligible) {
                          selectOrderFromHistory(o);
                        } else {
                          if (eligibility.reason === 'time_window') {
                            showIneligibleModal(o.name, 'time_window');
                          } else {
                            showIneligibleModal(o.name, 'custom');
                          }
                        }
                      }}
                      className={`
                        bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl border transition-all flex justify-between items-center group
                        ${!eligibility.eligible
                          ? 'border-slate-200 bg-slate-50/30 cursor-not-allowed opacity-75'
                          : 'border-slate-200 hover:border-pink-400 hover:shadow-md cursor-pointer'
                        }
                      `}
                    >
                      <div className="flex-1 min-w-0 mr-2">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-1">
                          <h3 className="text-base sm:text-lg font-bold text-slate-800 truncate">{o.name}</h3>
                          {orderStatus && orderStatus.status !== 'No Return' && (
                            <span className={`
                              text-[10px] sm:text-xs font-bold px-2 py-1 rounded-full w-fit
                              ${orderStatus.status === RETURN_STATUSES.CLOSED
                                ? 'bg-slate-100 text-slate-600 border border-slate-200'
                        ...
                        <div className="mt-2 flex gap-1.5 sm:gap-2 flex-wrap">
                          <span className="bg-emerald-50 text-emerald-700 text-[10px] sm:text-xs font-bold px-2 py-1 rounded border border-emerald-100">
                            Fulfilled
                          </span>
                          <span className="bg-slate-50 text-slate-600 text-[10px] sm:text-xs font-bold px-2 py-1 rounded border border-slate-100">
                            {o.line_items.length} Items
                          </span>
                          {isExpired && (
                            <span className="bg-orange-50 text-orange-600 text-[10px] sm:text-xs font-bold px-2 py-1 rounded border border-orange-200">
                              {daysSinceOrder} days ago
                            </span>
                          )}
                          {!eligibility.eligible && eligibility.reason === 'final_sale' && (
                            <span className="bg-red-50 text-red-600 text-[10px] sm:text-xs font-bold px-2 py-1 rounded border border-red-200">
                              Final Sale
                            </span>
                          )}
                        </div>
                      ...
                      <ChevronRight className={`w-4 h-4 sm:w-5 sm:h-5 shrink-0 ${
                        eligibility.eligible ? 'text-slate-300 group-hover:text-pink-500' : 'text-slate-300'
                      }`} />
                    </div>
                  );
                })}
                  ...
                  <div className="bg-white border border-slate-200 p-4 sm:p-5 lg:p-6 rounded-xl sm:rounded-2xl shadow-lg">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-5">
                      <div className="text-sm font-medium text-slate-600 flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-pink-600 text-lg sm:text-xl">
                      ...
                      <button
                        ...
                        disabled={submitting}
                        className="w-full sm:w-auto bg-slate-900 hover:bg-slate-800 text-white px-8 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                      >
                        {submitting ? (
                          <>
          ...
          {step === 'METHOD' && order && (
            <div className="max-w-2xl mx-auto space-y-4 sm:space-y-6 animate-in slide-in-from-right-8">
                  ...
                  <div className="flex items-start gap-3 sm:gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center">
                        ...
                        {selectedMethod === 'store_credit' && (
                          <div className="w-5 h-5 sm:w-6 sm:h-6 bg-pink-600 rounded-full flex items-center justify-center shrink-0 ml-2">
              ...
              <button
                ...
                className="w-full bg-slate-900 hover:bg-slate-800 text-white py-3 sm:py-4 rounded-xl font-bold shadow-lg transition-all flex items-center justify-center gap-2 mt-4 sm:mt-6 text-sm sm:text-base"
              >
          ...
          {step === 'REVIEW' && reviewData && order && (
            <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-right-8">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
                ...
                <div className="bg-blue-50 text-blue-700 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 sm:gap-2 w-fit">
              ...
              <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-200 overflow-hidden shadow-lg">
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-200">
                  <h3 className="font-semibold text-sm sm:text-base text-slate-800 flex items-center gap-2">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 bg-white rounded-lg flex items-center justify-center text-blue-600">
                ...
                {reviewData.items.map((item, index) => (
                  <div key={item.id} className="bg-white rounded-xl sm:rounded-2xl border border-slate-200 overflow-hidden shadow-lg">
                    <div className="bg-gradient-to-r from-slate-50 to-white px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-200 flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        <span className="w-5 h-5 sm:w-6 sm:h-6 bg-pink-100 text-pink-600 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-bold shrink-0">
                          {index + 1}
                        </span>
                        <h4 className="font-semibold text-sm sm:text-base text-slate-800 truncate max-w-[200px] sm:max-w-none">{item.title}</h4>
                      </div>
                      <span className="text-xs sm:text-sm font-bold text-pink-600 shrink-0 ml-2">Qty: {item.quantity}</span>
                    </div>
                    ...
                    <div className="p-4 sm:p-6">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
                        ...
                        <div>
                          ...
                          {item.productImage ? (
                            <div className="relative aspect-square rounded-lg border border-slate-200 overflow-hidden bg-slate-50">
                              <img src={item.productImage} alt="Product" className="w-full h-full object-cover" />
                            </div>
                          ) : (
                            <div className="bg-slate-50 rounded-lg p-6 sm:p-8 flex items-center justify-center">
              ...
              {/* Summary Card */}
              <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl sm:rounded-2xl border border-indigo-100 p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
                  <div>
                    <p className="text-[10px] sm:text-xs font-medium text-indigo-600 mb-1">Return Summary</p>
                    <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-slate-900">Product Amount</h3>
                  </div>
                  <div className="text-left sm:text-right w-full sm:w-auto">
                    <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-pink-600">{formatCurrency(reviewData.totalAmount)}</p>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2 sm:pt-4">
                ...
                <button
                  ...
                  disabled={submitting}
                  className="w-full sm:flex-1 bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-700 hover:to-purple-700 text-white font-bold py-3 sm:py-4 px-4 sm:px-6 rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed text-sm sm:text-base"
                >
                  {submitting ? (
                    <>
                      <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" />
            ...
            <div className="max-w-2xl mx-auto space-y-6 sm:space-y-8">
              <div className="text-center">
                <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 bg-emerald-100 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
                  <Check className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-emerald-600" />
                </div>
              ...
              <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-200 p-4 sm:p-6 lg:p-8 shadow-sm">
                ...
                <div className="space-y-3 sm:space-y-4">
                  <div className="flex justify-between items-center pb-2 sm:pb-3 border-b border-slate-100">
                    ...
                    <span className="text-xs sm:text-sm font-bold text-slate-900 font-mono">{generatedRAN}</span>
                  </div>
              ...
              <div className="text-center">
                <button
                  onClick={resetForm}
                  className="bg-slate-900 hover:bg-slate-800 text-white px-6 sm:px-8 py-2.5 sm:py-3 rounded-xl font-bold transition-all inline-flex items-center gap-2 shadow-lg hover:shadow-xl text-sm sm:text-base"
                >
                  Start New Return
                </button>
