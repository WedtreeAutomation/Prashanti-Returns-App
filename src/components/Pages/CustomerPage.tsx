...
import { fetchOrderByName, fetchProductDetails, fetchShopifyOrder, notifyReturnSubmission, logReturnToShopifyNote } from '../../Interfaces/api';
...
const MIN_NOTE_WORDS = 3;

// Flags keyboard-mash patterns like "aaaaaa" or "ababab" (a 1-3 char unit repeated 3+ times)
const isGibberishWord = (word: string): boolean => /^(.{1,3})\1{2,}$/i.test(word);

const isValidAdditionalNote = (note: string): boolean => {
  // Handle empty or undefined note
  ...
  if (words.length < MIN_NOTE_WORDS) return false;
  
  // Check each word is valid (at least 2 characters and not gibberish)
  return words.every(word => word.length >= 2 && !isGibberishWord(word));
};
...
export const ReturnPortalSplit = () => {
  ...
  const handleSpecificSearch = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Enter correct order number and email.");
    } finally {
  ...
  const validateReturn = useCallback((): boolean => {
    ...
    for (const itemId of selectedItems) {
      ...
      // Check if "Other reason" is selected and validate the note
      if (returnDetails[itemId]?.reason === OTHER_REASON) {
        const note = returnDetails[itemId]?.note || '';
        
        // Check if note is empty or doesn't meet minimum requirements
        if (!isValidAdditionalNote(note)) {
          const words = note.trim().split(/\s+/).filter(word => word.length > 0);
          const wordCount = words.length;
          
          alert(`Please describe the reason in at least ${MIN_NOTE_WORDS} words when selecting "Other reason". (Current: ${wordCount} word${wordCount !== 1 ? 's' : ''})`);
          return false;
        }
      }
  ...
  const handleSubmitReturn = useCallback(async () => {
    ...
    try {
      ...
      // 4. Process Items and Upload Images Concurrently
      ...
      // 7. Log to Shopify Timeline / Notes (Non-blocking)
      try {
        await logReturnToShopifyNote(order.id, processedItems, RAN);
      } catch (noteError) {
        console.error("Non-blocking error: Failed to post timeline note to Shopify", noteError);
      }

      // 8. Trigger Webhook and Update Status
      try {
    ...
    selectedMethod, 
    hasDefectiveItems, 
    generateSequentialRAN, 
    uploadFilesBatch, 
    orderReturnStatus, 
    extractCustomerContactDetails, 
    isOrderInIndia
  ]);
        ...
        {isSelected && !isDisabled && (
          <div className="px-3 sm:px-5 pb-4 sm:pb-6 pt-2 bg-slate-50/50 border-t border-[#5A0A38]/10">
            <div className="grid grid-cols-1 gap-3 sm:gap-4 mb-4 sm:mb-6">
              ...
              <div>
                <label className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase mb-1 block">
                  Additional Notes {details.reason === OTHER_REASON ? <span className="text-red-500">*</span> : '(Optional)'}
                </label>
                <input
                  type="text"
                  value={details.note}
                  onChange={(e) => updateDetail(item.id, 'note', e.target.value)}
                  placeholder={details.reason === OTHER_REASON ? "Please describe the reason (min 3 words)..." : "Add details..."}
                  className={`w-full p-2 sm:p-2.5 text-sm bg-white border rounded-xl outline-none focus:ring-2 focus:ring-[#5A0A38]/20 ${
                    details.reason === OTHER_REASON && !isValidAdditionalNote(details.note)
                      ? 'border-red-300 focus:border-red-400'
                      : 'border-slate-200 focus:border-[#5A0A38]'
                  }`}
                />
              </div>
            </div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm">
                  ...
                  <form onSubmit={handleSpecificSearch} className="space-y-2 sm:space-y-3">
                    <input
                      ...
                      placeholder="Order ID (e.g. P123456)"
