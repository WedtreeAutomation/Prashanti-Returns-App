...
const ItemComparisonCard = ({ item, index, RAN, onRejectClick }: { item: ReturnItem, index: number, RAN: string, onRejectClick: (itemId: number, item: ReturnItem) => void }) => {
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      for (let i = 0; i < item.customerImages.length; i++) {
        const response = await fetch(item.customerImages[i]);
        const blob = await response.blob();
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `${RAN}_${item.sku}_${i + 1}.jpg`;
  ...
  const handleDownloadProductImage = async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `${RAN}_${item.sku}_product.jpg`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
...
export const ReturnDetailsPage = () => {
  ...
  const fetchCustomerBalances = async (identifier: string, identifierType: 'email' | 'phone' = 'email'): Promise<CustomerBalances | null> => {
    try {
      ...
      const response = await fetch(`${apiUrl}/api/customer/balances`, {
        method: 'POST',
        headers: {
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button className="px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white text-xs font-medium rounded-lg flex items-center gap-1.5 shadow-md whitespace-nowrap"><RefreshCw className="w-3.5 h-3.5" /> Convert to Exchange</button>
