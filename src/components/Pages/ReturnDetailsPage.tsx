...
import { fetchShopifyOrder, restockShopifyItems, notifyReturnRejection, notifyItemRejection, apiClient, addShopifyNote, downloadImageFile, downloadImagesAsZip } from '../../Interfaces/api';
...
import { getUserFromStorage, RejectedItem, ActivityItem, ReturnItem, ReturnData, MarkReceivedItemDecision } from '../../Interfaces/types';
...
const ShipmentStatusBadge = ({ status }: { status?: string }) => {
  ...
  const statusConfig: Record<string, { color: string; bg: string; icon: React.ReactNode }> = {
    ...
    'self ship requested': { color: 'text-blue-700', bg: 'bg-blue-50 border-blue-100', icon: <Package className="w-3 h-3" /> },
...
const ItemComparisonCard = ({
  item, index, RAN, orderName, onRejectClick
}: { item: ReturnItem; index: number; RAN: string; orderName: string; onRejectClick: (itemId: number, item: ReturnItem) => void }) => {
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      const images = item.customerImages.map((url, i) => ({
        url,
        filename: `${item.sku || 'item'}_${i + 1}.jpg`
      }));
      await downloadImagesAsZip(images, `${orderName}_${RAN}_${item.sku || 'item'}_images`);
    } catch (error) {
      console.error('Error downloading images:', error);
      alert('Failed to download customer images. Please try again.');
    } finally {
  ...
  const handleDownloadProductImage = async () => {
    ...
    try {
      await downloadImageFile(item.productImage, `${RAN}_${item.sku || 'item'}_product.jpg`);
    } catch (error) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={handleDownloadAll} disabled={downloadingAll} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium flex items-center gap-1 px-2 py-1 hover:bg-indigo-50 rounded-lg transition-colors disabled:opacity-50">
                  <Download className="w-3 h-3" /> {downloadingAll ? 'Zipping…' : 'Download All'}
                </button>
              </div>
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={handleDownloadProductImage} disabled={downloadingProduct} className="text-xs text-slate-600 hover:text-slate-800 font-medium flex items-center gap-1 px-2 py-1 hover:bg-slate-100 rounded-lg transition-colors disabled:opacity-50">
                  <Download className="w-3 h-3" /> {downloadingProduct ? 'Downloading…' : 'Download'}
                </button>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  {item.note && (
                    <div className={`mt-3 p-3.5 bg-indigo-50 border-l-4 border-indigo-500 rounded-r-lg shadow-sm ${isExpanded ? 'block' : 'hidden'}`}>
                      <div className="flex items-start gap-2">
                        <FileText className="w-4 h-4 text-indigo-600 shrink-0 mt-0.5" />
                        <p className="text-sm text-indigo-900">
                          <span className="font-bold uppercase tracking-wider text-xs text-indigo-700 block mb-0.5">Customer Note</span>
...
export const ReturnDetailsPage = () => {
    ...
    try {
      ...
      if (itemIdToReject) {
        ...
        await addShopifyNote(
          data.orderNumericId || data.orderId,
          data.RAN,
          "Item Rejected",
          `Item: ${itemToReject.title} (SKU: ${itemToReject.sku})\nReason: ${reason}\nAction by: ${agentName}`
        ).catch(console.error);
  ...
  const handleMarkReceivedConfirm = async (decisions: MarkReceivedItemDecision[]) => {
    if (!id || !data) return;
    ...
    try {
      ...
      const decisionMap = new Map(decisions.map(d => [d.lineItemId, d]));

      const itemConditions: Record<number, string> = {};
      activeItems.forEach(item => {
        itemConditions[item.lineItemId] = decisionMap.get(item.lineItemId)?.condition || 'Unknown';
      });
      const uniqueConditions = Array.from(new Set(Object.values(itemConditions)));
      const overallCondition = uniqueConditions.length === 1 ? uniqueConditions[0] : 'Mixed';

      // Only items the agent chose to restock, at the agent-adjusted quantity
      const itemsToRestock = activeItems
        .map(item => {
          const decision = decisionMap.get(item.lineItemId);
          if (!decision || !decision.restock || decision.restockQuantity <= 0) return null;
          return {
            lineItemId: item.lineItemId,
            quantityReturned: Math.min(decision.restockQuantity, item.quantityReturned),
            sku: item.sku,
            title: item.title,
      ...
      if (itemsToRestock.length > 0) {
        try {
          // Fallback to parsed orderId if orderNumericId is missing/invalid
          const targetOrderId = data.orderNumericId || data.orderId;

          restockRecord = await restockShopifyItems(itemsToRestock, targetOrderId, id, data.RAN, agentName, overallCondition);
          updateData.isRestocked = true;
          updateData.restockStatus = itemsToRestock.length === activeItems.length ? 'Completed' : 'Partial';
          ...
          const errorMessage = restockError instanceof Error ? restockError.message : 'Unknown error';
          
          // CRITICAL FIX: Alert the user and abort the entire function so it isn't marked as received
          alert(`Failed to restock items: ${errorMessage}\n\nThe return has NOT been marked as received. Please fix the issue or uncheck the restock option to proceed.`);
          return; 
        }
      } else {
      ...
      const activitiesRef = collection(db, 'returns', id, 'activities');
      const conditionBreakdown = activeItems.map(item => `${item.title}: ${itemConditions[item.lineItemId]}`).join('; ');

      await addDoc(activitiesRef, {
        type: 'success',
        title: 'Return Received',
        description: `${activeItems.length} item(s) marked as received. ${restockRecord ? `Restocked ${restockRecord.totalQuantityRestocked} unit(s).` : 'No items restocked.'}`,
        timestamp: now,
        user: agentName,
        metadata: {
          itemConditions,
          restocked: itemsToRestock.length > 0 && restockRecord?.status === 'success',
      ...
      const restockMsg = restockRecord
        ? `Restocked ${restockRecord.totalQuantityRestocked} unit(s): ${restockRecord.items.map(i => `${i.title} x${i.quantityReturned}`).join(', ')}.`
        : 'No items were restocked.';

      await addShopifyNote(
        data.orderNumericId || data.orderId, data.RAN, "Returned Product Received",
        `Condition breakdown: ${conditionBreakdown}\n${restockMsg}\nAction by: ${agentName}`
      ).catch(console.error);

      setIsMarkReceivedModalOpen(false);
      await fetchReturnDetails();
      alert(`Return marked as received successfully. Processed ${activeItems.length} item(s). ${restockMsg}`);
    } catch (err) {
  ...
  const ActionMenu = () => {
    const menuActions = [
      { label: 'Create Pickup', icon: <Truck className="w-3.5 h-3.5" />, color: 'text-blue-600', action: 'pickup', disabled: !canCreatePickup },
      { label: 'Request Self Ship', icon: <Truck className="w-3.5 h-3.5" />, color: 'text-amber-600', action: 'self', disabled: !canCreatePickup },
      { label: 'Cancel Pickup', icon: <X className="w-3.5 h-3.5" />, color: 'text-red-600', action: 'cancel', disabled: !canCancelPickup },
    ];
    return (
        ...
        {menuActions.map((action, idx) => (
          <button
            ...
            onClick={() => {
              ...
              if (action.action === 'cancel') setIsCancelPickupModalOpen(true);
              else setActiveAction(action.action);
              setIsMenuOpen(false);
            }}
            className={`w-full text-left px-4 py-3 text-sm flex items-center gap-3 group transition-all duration-200 ${
              action.disabled ? 'opacity-40 cursor-not-allowed text-slate-400' : 'text-slate-700 hover:bg-slate-50'
            }`}
          >
            <div className={`p-1.5 rounded-lg bg-slate-100 group-hover:bg-white ${action.disabled ? '' : action.color}`}>{action.icon}</div>
  ...
  const shipmentLower = (data.shipmentStatus || '').toLowerCase();

  const isTerminal = ['completed', 'closed', 'denied'].includes(statusLower);
  const isShipmentActive = ['pickup created', 'pickup scheduled', 'in transit', 'self ship requested'].includes(shipmentLower);
  const isProductReached = shipmentLower === 'product reached' && !isTerminal;
  const isReceivedStage = shipmentLower === 'received' && !isTerminal;
  const isSelfShipRequested = shipmentLower === 'self ship requested' && !isTerminal;
  const canReject = !isReceivedStage && !isTerminal && !isProductReached;
  const canCreatePickup = !isShipmentActive && !isProductReached && !isReceivedStage && !isTerminal;
  const canCancelPickup = isShipmentActive || isProductReached;

  // IMPORTANT: Mark Received should ONLY be accessible for Product Reached status
  const canMarkReceived = (statusLower === 'approved' || statusLower === 'open') && (isProductReached || isSelfShipRequested) && !data.isRestocked;

  const canIssueRefund = isReceivedStage;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white p-4 sm:p-6">
      <div className="mb-8">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => canReject && setIsRejectModalOpen(true)}
              disabled={!canReject}
              className={`px-3 py-1.5 bg-white border border-slate-200 text-slate-700 hover:bg-red-50 hover:text-red-600 text-xs font-medium rounded-lg flex items-center gap-1.5 shadow-sm whitespace-nowrap ${!canReject ? 'opacity-40 cursor-not-allowed hover:bg-white hover:text-slate-700' : ''}`}
            ...
            <div className="relative">
              <button
                onClick={() => (canCreatePickup || canCancelPickup) && setIsMenuOpen(!isMenuOpen)}
                disabled={!canCreatePickup && !canCancelPickup}
                className={`px-3 py-1.5 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white text-xs font-medium rounded-lg flex items-center gap-1.5 shadow-md whitespace-nowrap ${(!canCreatePickup && !canCancelPickup) ? 'opacity-40 cursor-not-allowed' : ''}`}
              >
            ...
            <button
              onClick={() => canMarkReceived && setIsMarkReceivedModalOpen(true)}
              disabled={!canMarkReceived}
              className={`px-3 py-1.5 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white text-xs font-medium rounded-lg flex items-center gap-1.5 shadow-md whitespace-nowrap ${!canMarkReceived ? 'opacity-40 cursor-not-allowed' : ''}`}
            >
              <CheckCircle2 className="w-3.5 h-3.5" /> Mark Received
            </button>

            <button
              onClick={() => canIssueRefund && setIsIssueRefundModalOpen(true)}
              disabled={!canIssueRefund}
              className={`px-3 py-1.5 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white text-xs font-medium rounded-lg flex items-center gap-1.5 shadow-md whitespace-nowrap ${!canIssueRefund ? 'opacity-40 cursor-not-allowed' : ''}`}
            ><CreditCard className="w-3.5 h-3.5" /> Issue Refund</button>
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div>
              <label className="text-xs font-medium text-slate-500 block mb-1">Return Reason(s)</label>
              <div className="text-sm text-slate-800">
                {data.items && data.items.length > 0 ? (
                  Array.from(new Set(data.items.map(item => item.reason).filter(Boolean))).map((reason, index, array) => (
                    <span key={index}>
                      {reason}{index < array.length - 1 ? <span className="text-slate-400 mx-1">•</span> : ''}
                    </span>
                  ))
                ) : (
                  'Not specified'
        ...
        <div className="lg:col-span-9 space-y-6">
          <div className="space-y-4">
            ...
            {data.items.map((item, index) => (
              <ItemComparisonCard 
                key={index} item={item} index={index} RAN={data.RAN} orderName={data.orderId}
                onRejectClick={(itemId, item) => { setItemToReject({ id: itemId, item }); setIsRejectModalOpen(true); }}
              />
            ))}
          </div>
      ...
      <CreatePickupModal
        ...
        orderId={data.id}
        orderNumericId={data.orderNumericId} // <--- ADD THIS
        RAN={data.RAN}
