...
export interface RestockItem {
  ...
  variantId?: number;
  productId?: number;
  condition?: string; // per-item condition, independent of the overall record condition
}
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 403) {
      // CRITICAL FIX: Do not hijack the 403 if it's the customer verifying their order
      if (error.config?.url?.includes('/orders/verify')) {
...
export const addShopifyNote = async (
  orderNumericId: number | string, 
  ran: string, 
  eventTitle: string, 
  message: string
) => {
  try {
    ...
    await apiClient.post(`/orders/${orderNumericId}/order-note`, {
      ran,
      eventTitle,
      message
    });
  } catch (error) {
    console.error(`Failed to add Shopify note for ${eventTitle}:`, error);
  }
};

export const addShopifyTag = async (orderId: string | number, tag: string): Promise<boolean> => {
  try {
    const numericOrderId = typeof orderId === 'string' ? parseInt(orderId.replace(/\D/g, ''), 10) : orderId;
    if (!numericOrderId || isNaN(numericOrderId)) return false;

    const response = await apiClient.post(`/orders/${numericOrderId}/tags`, { tag });
    return response.data?.success || false;
  } catch (error) {
    console.error("Failed to add Shopify tag:", error);
    return false;
  }
};
...
export const restockShopifyItems = async (
  ...
  if (!items || items.length === 0) throw new Error('No items to restock');
  
  // CRITICAL FIX: Safely parse the order ID regardless of if it's passed as a string, number, or GraphQL gid.
  const numericOrderId = typeof orderId === 'string' ? parseInt(orderId.replace(/\D/g, ''), 10) : orderId;
  
  if (!numericOrderId || isNaN(numericOrderId)) throw new Error('Valid order ID is required');
  if (!returnId || !RAN) throw new Error('Return ID and RAN are required for tracking');

  const invalidItems = items.filter(item => !item.lineItemId || !item.quantityReturned);
  if (invalidItems.length > 0) throw new Error(`Invalid items: ${JSON.stringify(invalidItems)}`);
  ...
  try {
    const response = await apiClient.post(`/orders/${numericOrderId}/restock`, {
      items: items.map(item => ({ lineItemId: item.lineItemId, quantityReturned: item.quantityReturned }))
    });
    ...
    if (error.response?.data?.error) throw new Error(`Restock failed: ${error.response.data.error}`);
    if (error.response?.status === 404) throw new Error(`Order ${numericOrderId} not found`);
    if (error.response?.status === 400) throw new Error(`Invalid request: ${error.response.data?.error || 'Check item data'}`);
    throw error;
  }
};
/**
...
export const notifyRefundDone = async (
  ...
  refundDetails: {
    ...
    expiryDate?: string;
    // 👇 ADDED: Fields to pass breakdown data to the webhook
    baseAmount?: number;
    shippingRefundAddition?: number;
    deductions?: {
      restocking: number;
      reverseShipment: number;
      forwardShipment: number;
      adjustment: number;
      taxes: number;
    };
    quantityMultiplied?: {
...
export const downloadImageFile = async (url: string, filename: string): Promise<void> => {
  const response = await apiClient.get('/download/image', {
    params: { url, filename },
    responseType: 'blob'
  });
...
export const logReturnToShopifyNote = async (
  orderId: number,
  items: Array<{ title: string; sku?: string | null; quantityReturned: number; reason?: string }>,
  ran: string
): Promise<boolean> => {
  try {
    const itemsSummary = items
      .map(i => `• ${i.title} (SKU: ${i.sku || 'N/A'}) x${i.quantityReturned} — Reason: ${i.reason || 'N/A'}`)
      .join('\n');

    await apiClient.post(`/orders/${orderId}/order-note`, {
      ran,
      eventTitle: 'Return Request Submitted',
      message: `Customer submitted a return request for ${items.length} item(s):\n${itemsSummary}`
    });
    return true;
  } catch (error) {
    console.error("Failed to log return note to Shopify:", error);
    return false;
  }
};
...
export const loginAgent = async (email: string, passwordOrPhone: string) => {
  const response = await apiClient.post('/auth/login', { email, password: passwordOrPhone });
  return response.data;
};

export const setupAgentPassword = async (email: string, password: string) => {
  const response = await apiClient.post('/auth/setup-password', { email, password });
  return response.data;
};
