...
import { notifyPickupCreated, addShopifyNote, addShopifyTag } from '../Interfaces/api';
import {
  collection,
  getDocs,
  doc,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        const providerDetail = data.response?.GenerateWayBillResult?.Status?.[0]?.StatusInformation
          || data.response?.Status?.[0]?.StatusInformation;
        const detailedMessage = providerDetail
          ? `${data.error || 'Failed to schedule pickup'}: ${providerDetail}`
          : (data.error || 'Failed to schedule pickup');
        console.error('BlueDart raw response:', data.response);
      ...
      // --- ADD SHOPIFY NOTE FOR PICKUP CREATION ---
      try {
        const targetShopifyId = orderNumericId || getNumericOrderId(orderId);
        
        if (targetShopifyId) {
          const itemsSummary = normalizedItems
            .map(item => `• ${item.title} (SKU: ${item.sku || 'N/A'}) x${item.quantityReturned}`)
            .join('\n');

          const noteMessage = `Pickup scheduled for ${normalizedItems.length} item(s):\n${itemsSummary}\nAWB: ${data.awbNumber}\nPickup Date: ${pickupDate.toLocaleDateString()}\nScheduled by: ${currentUser?.name || 'System'}`;
          
          // 1. Add the Timeline Note
          await addShopifyNote(targetShopifyId, RAN, "Pickup Created", noteMessage);
          
          // 2. ✅ ADD THE SHOPIFY TAG
          await addShopifyTag(targetShopifyId, "Return Approved Created Pickup");
        }
      } catch (shopifyError) {
        console.error("Failed to add Shopify note or tag:", shopifyError);
        // We don't block the UI if Shopify tracking fails, as the waybill was already created
      }
